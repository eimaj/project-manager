...
- **Project-local briefs.** `<root>/briefs/` (seeded here; path in `.pm/config.json` → `briefs_dir`, default `<root>/briefs/`) holds `/orchestrate-brief` output for this project, named `YYYY-MM-DD-<TICKET>-<slug>.md`. `/orchestrate-brief` writes to the orchestrate default `{artifact_root}/runs/<session_id>/`; resolve `briefs_dir` and relocate the brief there — do **not** change orchestrate's `artifact_root`.
